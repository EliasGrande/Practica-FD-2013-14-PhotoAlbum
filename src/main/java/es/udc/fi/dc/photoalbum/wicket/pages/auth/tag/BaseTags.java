...
public class BaseTags extends BasePageAuth{
	...
	public BaseTags(PageParameters parameters) {
		...
		this.tag = tag;
		add(new AjaxDataView("dataAlbumContainer", "albumNavigator", createAlbumDataView()));
		add(new AjaxDataView("dataFileContainer", "fileNavigator", createFileDataView()));
	}
	...
	private DataView<Album> createAlbumDataView() {
		...
		DataView<Album> dataView = new DataView<Album>("pageable",
				new TagAlbumListDataProvider(ldm.getObject().size(),userId,tag)) {
	...
	private DataView<File> createFileDataView() {
		int userId = ((MySession) Session.get()).getuId();
		LoadableDetachableModel<ArrayList<File>> ldm = new TagFilesModel(tag,userId);
